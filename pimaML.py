...
print(f"Accuracy on training set before grid search: {results.mean()*100.0}%")
...
print(f"Best hyperparameters determined by grid search: {grid.best_params_}")
gs.fit(train_X, train_y)

#trains model using new hyperparameters
scores_new = cross_val_score(gs, train_X, train_y, cv = kfold)
print(f'Accuracy score mean after grid search: {scores_new.mean()*100}%')

#evaluates on test dataset
accuracy_score_test = gs.score(test_X, test_y)
print(f'Accuracy score on the test set post grid search: {accuracy_score_test*100}%')
